...
class GameController {
    ...
    goHome() {
        ...
        if (this.tvCtx) this.tvCtx.clearRect(0, 0, this.tvCanvas.width, this.tvCanvas.height);
    ...
    endGame(success) {
        ...
        const gameIdForResult = this.currentGameId;
        const isAllyGame = gameIdForResult >= 11;

        if (isAllyGame) {
            // NEW: Keep game-screen visible, show dim overlay on top after short delay
            setTimeout(() => {
                // Show result overlay (dim + card + buttons) over the game canvas
                resultScreen.show(success, gameIdForResult);

                // Sync TV to RESULT state (TV overlay is handled by resultScreen)
                // Keep tv-game-screen canvas visible — don't switch to tv-result-screen
            }, 300);
        } else {
            // LEGACY: Switch to result screen
            setTimeout(() => {
                this.gameScreen.classList.remove('active');
                resultScreen.show(success, gameIdForResult);
                this.syncTVScreens();
